...
**请求**:
```bash
curl -X POST "http://localhost:8080/api/ragcom/ingestion/pipelines" \
...
**请求**:
```bash
curl -X POST "http://localhost:8080/api/ragcom/ingestion/tasks/upload" \
  -F "pipelineId=1" \
...
**请求**:
```bash
curl "http://localhost:8080/api/ragcom/ingestion/tasks/123"
```
...
API_BASE="http://localhost:8080/api/ragcom"
